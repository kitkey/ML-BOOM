...
class SwiGLU(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        silu_out = self.silu(gate_out)
        up_out = self.up(x)
        union_out = up_out * silu_out
        down_out = self.down(union_out)
        drop_out = self.dropout(down_out)
...
class RoPE(nn.Module):
    def __init__(self, head_size: int, max_seq_len: int, base: float = 10000.0):
        super().__init__()
        self.head_size = head_size
        self.max_seq_len = max_seq_len
        self.base = base

        inv_freq = base ** (-torch.arange(0, head_size, 2, dtype=torch.float32) / head_size)
        positions = torch.arange(max_seq_len, dtype=torch.float32)
        angles = positions[:, None] * inv_freq[None, :]

        self._rotation_matrix_cos = torch.cos(angles)
        self._rotation_matrix_sin = torch.sin(angles)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        batch, seq_len, head_size = x.shape

        cos_rotation = self._rotation_matrix_cos[:seq_len][None, :, :]
        sin_rotation = self._rotation_matrix_sin[:seq_len][None, :, :]

        x_even = x[..., 0::2]
        x_odd = x[..., 1::2]

        x_even_rot = x_even * cos_rotation - x_odd * sin_rotation
        x_odd_rot = x_odd * cos_rotation + x_even * sin_rotation
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: torch.FloatTensor):
        norma_output = torch.sqrt(torch.mean(x ** 2, dim=-1, keepdim=True) + self.eps)
        rms_output = self.weight * x / norma_output
        return rms_output
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        self.module_list = nn.ModuleList(
            [HeadAttention(emb_size, head_size, max_seq_len, rope) for _ in range(num_heads)]
        )
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.ff = SwiGLU(emb_size, dropout)
        self.first_ln = RMSNorm(emb_size)
        self.second_ln = RMSNorm(emb_size)
...
class Llama(nn.Module):
    def __init__(
        ...
        self.decoder_blocks = nn.ModuleList(
            [Decoder(num_heads, emb_size, head_size, max_seq_len, self.rope, dropout) for _ in range(num_layers)]
        ).to(device)
        self.decoder_norm_layer = RMSNorm(emb_size).to(device)
    ...
    def forward(self, x: torch.Tensor, use_cache: bool = True, cache: Optional[List[List[Tuple[torch.Tensor]]]] = None):
        ...
        drop_out = self.dropout(token_emb)
    ...
    def generate(
        ...
        for iteration in range(max_new_tokens):
            ...
            if use_cache:
                ...
                logits = self.forward(x_restr, use_cache=use_cache)[0]
